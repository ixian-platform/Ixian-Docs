...
# addTestnetPeer = seedtest1.ixian.io:11234
# Regtest (local Docker sandbox):
# addTestnetPeer = 10.24.231.1:10234

# API settings
apiPort = 8001                      # REST API port
apiBind = http://127.0.0.1:8001/         # Bind to localhost (use 0.0.0.0 for public)
...
# Stream capabilities - set incoming (1) + apps (8) + app protocols (16) = 25
streamCapabilities = 25
...
- Use `apiBind = http://127.0.0.1:8001/` unless you need external access
...
- Try different seed nodes in `addPeer` or `addTestnetPeer` setting
